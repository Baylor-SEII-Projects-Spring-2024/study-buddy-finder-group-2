        ...
        useEffect(() => {
            const observer = new IntersectionObserver(
                ...
                },
                {
                    ...
                    rootMargin: '-30px',  // Can be adjusted to 'negative' values if the arrow flickers out too soon
                    threshold: 0.0  // Trigger as soon as even one pixel is visible or not
                }
            );
                    ...
                    entries.forEach(entry => {
                        if (entry.target === meetupsRef.current) {
                            if (entry.isIntersecting) {
                                setHeaderVisible(true); // Fade in when entering the viewport
                            } else {
                                setHeaderVisible(false); // Fade out when leaving the viewport, allows re-triggering
                            }
                        }
                        if (entry.target === arrowRef.current) {
                            setShowArrow(!entry.isIntersecting); // Hide arrow when intersecting, show when not
                        }
                    });
                },
                {
